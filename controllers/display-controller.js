...
export default (() => {
    ...
    const displayProjects = (projects) => {
        ...
        for (const project of projects) {
            ...
            deleteProjectButton.addEventListener('click', e => deleteProject(e, project.getId()));
    ...
    const createProjectWrapper = (project) => {
        ...
        for (const task of tasks) {
            ...
            taskContent.addEventListener('click', e => toggleTaskIsCompleted(project.getId(), e));

            const deleteTaskButton = document.createElement('button');
            deleteTaskButton.classList.add('delete-task');
            deleteTaskButton.textContent = 'X';
            deleteTaskButton.addEventListener('click', e => deleteTask(project.getId(), task.getId()));
    ...
    const saveNewTask = event => {
        ...
        const tasks = TodoController.saveTask(TodoController.getProjectWithId(projectId));
        displayProjectDetails(tasks, projectId);
        taskDialog.close();
    }
    ...
    const assignEventListeners = () => {
        ...
        const projectDialogCancelButton = document.querySelector('button#project-dialog-cancel');
        projectDialogCancelButton.addEventListener('click', closeProjectDialog);

        const projectDialogSaveButton = document.querySelector('button#project-dialog-save');
        projectDialogSaveButton.addEventListener('click', saveNewProject);

        const taskDialogCancelButton = document.querySelector('button#task-dialog-cancel');
        taskDialogCancelButton.addEventListener('click', closeTaskDialog);

        const taskDialogSaveButton = document.querySelector('button#task-dialog-save');
        taskDialogSaveButton.addEventListener('click', saveNewTask);
    }

    return {
        assignEventListeners, displayProjects
