...
export default (() => {
  let projects = [
    Project(
      "https://d1yhils6iwh5l5.cloudfront.net/charts/resized/63221/normal/z_hedgeye_goos.png",
      "Save the birds",
    ...
    Project(
      "https://thumbnails.cbc.ca/maven_legacy/thumbnails/488/667/national1_2500kbps_852x480_1006631491920.jpg?crop=1.777xh:h;*,*&downsize=560px:*",
      "Kayak championship",
  ...
  const saveProject = () => {
    ...
    const priority = formElement.querySelector(
      'dialog input[type="radio"][name="priority"]:checked'
    ).value;
    const dueDate = formElement.querySelector(
    ...
    if (title && description && priority && iconUrl) {
      projects.push(
        Project(iconUrl, title, description, priority, dueDate, projectId)
      );
    } else {
